...
module Syntax (
    ...
    subst, absArgType, applyAbs, scopelessApplyAbs, scopelessApplyNaryAbs,
    -- makeAbs,
...
data WithBindings e n where
  WithBindings :: BindingsFrag n l -> e l -> WithBindings e n
type EvaluatedModule = WithBindings SourceNameMap  :: * -> *

emptyEvaluatedModule :: EvaluatedModule n
emptyEvaluatedModule = WithBindings emptyEnvFrag mempty

instance HasNames Module
...
-- TODO: make this just `Env i (Atom o)` if we remove type annotations from
-- variable occurrences
type SubstEnv     i o    = Env i (Either (Name o) (Atom o))
type SubstEnvFrag i i' o = EnvFrag i i' (Atom o)
type Bindings n       = Env n (Type n, BinderInfo n)
type BindingsFrag n l = EnvFrag n l (Type l, BinderInfo l)

type ScopedSubstEnv i o = (Scope o, SubstEnv i o)

idSubstEnv :: Scope n -> SubstEnv n n
idSubstEnv scope = Env scope emptyEnvFrag Left
...
-- instance BindsVars a => HasNames (Nest a) where
...
applyAbs scope (Abs b body) x = subst env body
  where env = (scope, idSubstEnv scope <>> b @> Right x)
...
instance (HasNames e) => HasNames (WithArrow e)
instance (AlphaEq e)  => AlphaEq (WithArrow e)
instance (Subst e)    => Subst (WithArrow e)

instance BindsVars Binder

instance (BindsVars b) => BindsVars (Nest b)

instance (BindsVars b, Subst e) => Subst (Abs b e)
