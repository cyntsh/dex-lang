...
requiredFunctions :: HasNames e => Scope n -> e n -> [(Name n, Atom n)]
  ...
  Case e alts _ -> do
    ...
    case e' of
      DataCon _ _ con args -> do
        case alts !! con of
          Abs bs body -> translateBlock (env <>> bs @@> map Right args) (maybeDest, body)
      Variant (NoExt types) label i x -> do
        let LabeledItems ixtypes = enumerate types
        let index = fst $ ixtypes M.! label NE.!! i
        case alts !! index of
          Abs bs body -> translateBlock (env <>> bs @@> [Right x]) (maybeDest, body)
      Con (SumAsProd _ tag xss) -> do
        ...
        emitSwitch tag' $ flip map (zip xss alts) $
          \(xs, Abs bs body) ->
             void $ translateBlock (env <>> bs @@> map Right xs) (Just dest, body)
        destToAtom dest
  ...
  case hof of
          ...
          case dev of
            CPU -> do -- Chunked loop
              ...
              emitLoop "li" Fwd (fromScalarAtom threadChunkSize) \li -> do
                ...
                void $ translateBlock (env <>> b @> Right idx) (Just ithDest, body)
    ...
    While (Lam (Abs b (WithArrow _ body))) -> do
      body' <- liftM snd $ scopedBlock $ do
                 ans <- translateBlock (env <>> b@>Right UnitVal) (Nothing, body)
    ...
    RunReader r (BinaryFunVal h ref _ body) -> do
      ...
      copyAtom rDest r'
      let env' = env <>> h @> undefined <>> ref @> Right rDest
    ...
    RunState s (BinaryFunVal h ref _ body) -> do
      ...
      let env' = env <>> h @> undefined <>> ref @> Right sDest
      void $ translateBlock env' (Just aDest, body)
      PairVal <$> destToAtom aDest <*> destToAtom sDest
    RunIO (Lam (Abs b (WithArrow _ body))) ->
      translateBlock (env <>> b@>Right UnitVal) (maybeDest, body)
