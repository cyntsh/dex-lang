...
checkType = undefined
-- checkType env eff x = void $ ctx $ runTypeCheck tyCheckEnv $ typeCheck x
--   where
--     tyCheckEnv = (idSubstEnv (fst env), CheckWith (env, eff))
--     ctx = addContext $ "Checking:\n" ++ pprint x
...
checkEq :: (ShowH e, PrettyH e, AlphaEq e) => e o -> e o -> TypeM i o ()
checkEq = undefined
